(function () {
  const dictionary = {
    en: {
      ...
      ident_title: "One mailbox, several addresses",
      ...
      ident_proven_title: "proven",
      ident_proven_desc: "The address appears in the From: of something in your Sent folder. The server has already accepted sending as it \u2014 that is history, not a guess.",
      ident_receives_title: "receives",
      ident_receives_desc: "The address appears in the To: of mail that arrived. It proves the mailbox receives on it, which is not the same as being allowed to send from it.",
      ident_reply: "Replies pick the identity on their own: a message that arrived for you@yourdomain.com is answered from you@yourdomain.com, not from whichever address you happened to log in with.",
      scope_title: "What the agent sees",
      scope_intro: "A mailbox that receives on several addresses is also, unavoidably, one IMAP connection. Opening \"the agent's mailbox\" and opening your personal mail are the same connection, so without a limit the very first listing hands the agent fifteen hundred messages that were never meant for it. Every account is created scoped to the address you connected with.",
      scope_both: "The limit holds on both sides: the server-side search is filtered before results exist, and reading by UID checks again \u2014 a UID can come from anywhere, including a guess.",
    ...
    pt: {
      ...
      ident_title: "Uma caixa, vários endereços",
      ident_intro: "Uma conta do iCloud não é um endereço: é uma caixa que recebe por vários \u2014 o Apple ID, os @icloud.com, o domínio próprio, os apelidos do Ocultar Meu E-mail. Todos caem na mesma entrada, então conectar cada um como conta separada mostraria a mesma correspondência três vezes. O que muda de verdade é por qual endereço a mensagem sai.",
      ident_proven_title: "provado",
      ident_proven_desc: "O endereço aparece no From: de algo na pasta Enviados. O servidor já aceitou mandar por ele \u2014 é histórico, não palpite.",
      ident_receives_title: "recebe",
      ident_receives_desc: "O endereço aparece no To: do que chegou. Prova que a caixa recebe por ele, o que não é a mesma coisa que poder enviar.",
      ident_reply: "A resposta escolhe sozinha: uma mensagem que chegou para voce@seudominio.com é respondida por voce@seudominio.com, e não pelo endereço com que você conectou.",
      scope_title: "O que o agente enxerga",
      scope_intro: "Uma caixa que recebe por vários endereços é, inevitavelmente, uma conexão IMAP só. Abrir \"a caixa do agente\" e abrir a sua correspondência pessoal são a mesma conexão \u2014 sem trava, a primeira listagem entrega ao agente mil e quinhentas mensagens que nunca foram para ele. Toda conta nasce presa ao endereço com que você conectou.",
      scope_both: "A trava vale nos dois lados: a busca sai filtrada do servidor, antes de o resultado existir, e ler por UID confere de novo \u2014 um UID pode vir de qualquer lugar, inclusive de um palpite.",
